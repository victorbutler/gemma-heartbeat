volatile uint16_t rate[10];                    // used to hold last ten IBI values
volatile uint16_t sampleCounter = 0;          // used to determine pulse timing
volatile uint16_t lastBeatTime = 0;           // used to find the inter beat interval
volatile uint16_t P = 512;                     // used to find peak in pulse wave
volatile uint16_t T = 512;                     // used to find trough in pulse wave
volatile uint16_t thresh = 512;                // used to find instant moment of heart beat
volatile uint16_t amp = 100;                   // used to hold amplitude of pulse waveform
